...
In the very night of Taproot activation, actually early morning for me, Taproot activated and we saw a lot of people broadcasting their first Taproot spends into the `mempool`, including up-to-time messages saying I'm the first one to spend Taproot, for example.
...
Oh yeah, right, Okay, so yeah, even before activation actually, we did with `F2Pool`, we spent a few Taproot outputs that were like low value, related them to brink just to show and learn how that's done and show that the software activates actually anyone can spend or are not yet unspendable.

Andrew Chow: 00:06:48
...
And then after a few weeks, the Ant pool took a bit longer.
...
In December 2020, I sent `PR` to `Bitcoin Core`, adding very primitive support for that.
And I picked that up, that work, and added the first `trace points` that to `Bitcoin Core` that are reached in the networking layer.
